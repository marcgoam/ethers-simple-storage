const ethers = require("ethers")
const fs = require("fs-extra")
require("dotenv").config()

async function main() {
    //htpp://127.0.0.1:7545
    const provider = new ethers.JsonRpcProvider(process.env.RPC_URL)
    const wallet = new ethers.Wallet(process.env.PRIVATE_KEY, provider)
    const abi = fs.readFileSync(
        "./SimpleStorage_sol_SimpleStorage.abi",
        "utf-8"
    )
    const binary = fs.readFileSync(
        "./SimpleStorage_sol_SimpleStorage.bin",
        "utf-8"
    )
    const contractFactory = new ethers.ContractFactory(abi, binary, wallet)
    console.log("Deploying, please wait...")
    const contract = await contractFactory.deploy() // Wait for contract to deploy!
    const deploytransaction = await contract.deploymentTransaction().wait()
    console.log("contract deployed to: ", deploytransaction.contractAddress)
    // console.log(`Contract Address: ${(await contract.getAddress()).toString()}`)
    // const deploymentReceipt = await contract.deploymentTransaction(1);

    let currentFavoriteNumber = (await contract.retrieve()).toString()
    console.log(`Current Favorite Number: ${currentFavoriteNumber.toString()}`)
    console.log("Updating favorite number...")
    let transactionResponse = await contract.store(7)
    await transactionResponse.wait(10)
    currentFavoriteNumber = (await contract.retrieve()).toString()
    console.log(`New Favorite Number: ${currentFavoriteNumber}`)
}

main()
    .then(() => process.exit(0))
    .catch((error) => {
        console.log(error)
        process.exit(1)
    })
